...
class bdxTypesBaseTest(aiounittest.AsyncTestCase):
    """Simple tests"""

    def __init__(self, *args, **kwargs):
        super(bdxTypesBaseTest, self).__init__(*args, **kwargs)
    ...
    async def test_simple_msg(self):
        ...
        test_deser = await ar2.message(None, msg_type=bdx.TxinGen)
    ...
    async def test_simple_msg_into(self):
        ...
        msg2 = bdx.TxinGen()
        ar2 = x.Archive(MemoryReaderWriter(writer.get_buffer()), False)
        test_deser = await ar2.message(msg2, bdx.TxinGen)
        self.assertEqual(msg.height, test_deser.height)
    ...
    async def test_ecpoint(self):
        ...
        await x.dump_blob(writer, ec_data, bdx.ECPoint)
        self.assertTrue(len(writer.get_buffer()), bdx.ECPoint.SIZE)

        test_deser = await x.load_blob(x.MemoryReaderWriter(writer.get_buffer()), bdx.ECPoint)
    ...
    async def test_ecpoint_obj(self):
        ...
        """
        ec_data = bytearray(range(32))
        ec_point = bdx.ECPoint(ec_data)
        writer = x.MemoryReaderWriter()

        await x.dump_blob(writer, ec_point, bdx.ECPoint)
        self.assertTrue(len(writer.get_buffer()), bdx.ECPoint.SIZE)

        ec_point2 = bdx.ECPoint()
        test_deser = await x.load_blob(x.MemoryReaderWriter(writer.get_buffer()), bdx.ECPoint, elem=ec_point2)
    ...
    async def test_tuple(self):
        ...
        out_entry = [
            123,
            bdx.CtKey(dest=self.test_data.generate_ec_key(), mask=self.test_data.generate_ec_key()),
        ]
        writer = MemoryReaderWriter()
        ar1 = x.Archive(writer, True)

        await ar1.tuple(out_entry, bdx.OutputEntry)
        ar2 = x.Archive(MemoryReaderWriter(writer.get_buffer()), False)
        test_deser = await ar2.tuple(None, bdx.OutputEntry)
    ...
    async def test_txin_to_key(self):
        ...
        ar2 = x.Archive(MemoryReaderWriter(writer.get_buffer()), False)
        test_deser = await ar2.message(None, bdx.TxinToKey)
    ...
    async def test_txin_variant(self):
        ...
        ar2 = x.Archive(MemoryReaderWriter(writer.get_buffer()), False)
        test_deser = await ar2.variant(None, bdx.TxInV, wrapped=True)
        self.assertEqual(test_deser.__class__, bdx.TxInV)
        self.assertEqual(msg, test_deser)
    ...
    async def test_tx_prefix(self):
        ...
        writer = MemoryReaderWriter()
        ar1 = x.Archive(writer, True, bdx.hf_versions(9))
        await ar1.message(msg)

        ar2 = x.Archive(MemoryReaderWriter(writer.get_buffer()), False, bdx.hf_versions(9))
        test_deser = await ar2.message(None, bdx.TransactionPrefix)
        self.assertEqual(test_deser.__class__, bdx.TransactionPrefix)
    ...
    async def test_boro_sig(self):
        ...
        ar2 = x.Archive(MemoryReaderWriter(writer.get_buffer()), False, bdx.hf_versions(9))
        test_deser = await ar2.message(None, bdx.BoroSig)
        self.assertEqual(msg, test_deser)
    ...
    async def test_transaction_prefix(self):
        ...
        ar1 = x.Archive(reader, False, bdx.hf_versions(9))

        test_deser = await ar1.message(None, bdx.TransactionPrefix)
    ...
    async def test_transaction(self):
        ...
        tsx_bin = base64.b16decode(tsx_hex, True)
        reader = x.MemoryReaderWriter(bytearray(tsx_bin))
        ar = x.Archive(reader, False, bdx.hf_versions(9))
    ...
    async def test_tx_unsigned(self):
        ...
        writer = x.MemoryReaderWriter()
        ar2 = x.Archive(writer, True, bdx.hf_versions(9))
        await ar2.root()
    ...
    async def test_versions(self):
        dest = bdx.TxDestinationEntry()
        dest.original = "test"
        dest.amount = 1234
        dest.addr = bdx.AccountPublicAddress(m_spend_public_key=bytearray(32), m_view_public_key=bytearray(32))
        dest.is_integrated = True
        dest.is_subaddress = True

        writer = x.MemoryReaderWriter()
        ar2 = x.Archive(writer, True, bdx.hf_versions(10))
        await ar2.root()
        await ar2.message(dest)

        reader = x.MemoryReaderWriter(writer.get_buffer())
        ar = x.Archive(reader, False, bdx.hf_versions(10))
        msg = bdx.TxDestinationEntry()
        await ar.root()
        await ar.message(msg)
        ...
        ar2 = x.Archive(writer2, True, bdx.hf_versions(9))
