...
# -*- coding: utf-8 -*-
'''
Minimal streaming codec for a Beldex binary serialization.
...
def import_def(module, name):
    if module not in sys.modules:
        if not module.startswith("beldex_serialize"):
...
class Archive(object):
    ...
    Resembles Archive API from the Beldex codebase or Boost serialization archive.
    ...
    def _get_type(self, elem_type):
        ...
        if issubclass(elem_type, bdxType):
            return elem_type
