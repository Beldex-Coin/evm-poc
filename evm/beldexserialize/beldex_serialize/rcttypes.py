...
from . import bdxtypes as type

class Key64(x.ContainerType):
    FIX_SIZE = 1
    SIZE = 64
    BOOST_RAW_ARRAY = True
    ELEM_TYPE = type.ECKey
    ...
    async def serialize_rctsig_base(self, ar, inputs, outputs):
        ...
        for i in range(outputs):
            if self.type in (RctType.Bulletproof2, RctType.CLSAG, RctType.BulletproofPlus):
                ...
                await ar.field(eref(am8, 0), type.Hash8)
                if not ar.writing:
        ...
        for i in range(outputs):
            await ar.field(eref(self.outPk[i], 'mask'), type.ECKey)
        await ar.end_array()
    ...
    async def serialize_rctsig_prunable(self, ar, type, inputs, outputs, mixin):
        ...
        if type in (RctType.CLSAG, RctType.BulletproofPlus):
            ...
            for i in range(inputs):
                ...
                await ar.tag('c1')
                await ar.field(eref(self.CLSAGs[i], 'c1'), elem_type=type.ECKey)
                await ar.tag('D')
                await ar.field(eref(self.CLSAGs[i], 'D'), elem_type=type.ECKey)
                await ar.end_object()
            ...
            for i in range(mg_elements):
                ...
                await ar.tag('cc')
                await ar.field(eref(self.MGs[i], 'cc'), elem_type=type.ECKey)
                await ar.end_object()
            await ar.end_array()
...
class Signature(x.MessageType):
    ...
    async def serialize_archive(self, ar, version=None):
        ar.field(eref(self, 'c'), type.ECKey)
        ar.field(eref(self, 'r'), type.ECKey)
        return self
...
def get_signature_size(msg):
    ...
    :return:
    """
    if isinstance(msg, (type.TxinGen, type.TxinToScript, type.TxinToScriptHash)):
        return 0
    elif isinstance(msg, type.TxinToKey):
        return len(msg.key_offsets)
    else:
        raise ValueError('Unknown tx in')


class Transaction(type.TransactionPrefix):
    # noinspection PyTypeChecker
    MFIELDS = type.TransactionPrefix.MFIELDS + [
    ...
    async def serialize_archive(self, ar, version=None):
        ...
        await ar.message(self, type.TransactionPrefix)
        ...
        if self.version == 1:
            ...
            if self.rct_signatures.type != RctType.Null:
                mixin_size = len(self.vin[0].key_offsets) - 1 if len(self.vin) > 0 and isinstance(self.vin[0], type.TxinToKey) else 0
    ...
    async def boost_serialize(self, ar, version):
        await ar.message(self, type.TransactionPrefix, use_version=version)
...
class AccountPublicAddress(x.MessageType):
    ...
    MFIELDS = [
        ('m_spend_public_key', type.ECPublicKey),
        ('m_view_public_key', type.ECPublicKey),
    ]
...
class MultisigInfo(x.MessageType):
    ...
    BOOST_VERSION = 1
    MFIELDS = [
        ('signer', type.ECPublicKey),
        ('LR', x.ContainerType, MultisigLR),
        ('partial_key_images', x.ContainerType, type.KeyImage),
    ]
...
class MultisigStruct(x.MessageType):
    ...
    MFIELDS = [
        ('sigs', RctSig),
        ('ignore', type.ECPublicKey),
        ('used_L', x.ContainerType, type.ECKey),
        ('signing_keys', x.ContainerType, type.ECPublicKey),
        ('msout', MultisigOut),
    ]
...
class TxExtraAdditionalPubKeys(x.MessageType):
    ...
    VARIANT_CODE = 0x4
    MFIELDS = [
        ('data', x.ContainerType, type.ECPublicKey),
    ]
...
class TxSourceEntry(x.MessageType):
    ...
    MFIELDS = [
        ...
        ('real_output', x.SizeT),
        ('real_out_tx_key', type.ECPublicKey),
        ('real_out_additional_tx_keys', x.ContainerType, type.ECPublicKey),
...
class TransferDetails(x.MessageType):
    ...
    MFIELDS = [
        ...
        ('m_multisig_k', x.ContainerType, type.ECKey),
...
class PendingTransaction(x.MessageType):
    ...
    MFIELDS = [
        ...
        ('tx_key', type.SecretKey),
        ('additional_tx_keys', x.ContainerType, type.SecretKey),
        ('dests', x.ContainerType, TxDestinationEntry),
...
class SignedTxSet(x.MessageType):
    ...
    MFIELDS = [
        ...
        ('key_images', x.ContainerType, type.KeyImage),
    ]
...
class MultisigTxSet(x.MessageType):
    ...
    MFIELDS = [
        ...
        ('m_signers', type.UnorderedSet, type.ECPublicKey),
    ]
...
class AccountKeys(x.MessageType):
    MFIELDS = [
        ...
        ('m_spend_secret_key', type.SecretKey),
        ('m_view_secret_key', type.SecretKey),
        ('m_multisig_keys', x.ContainerType, type.SecretKey),
    ]
    ...
    async def kv_serialize(self, ar, obj=None, **kwargs):
        ...
        await ar.message_field(self, ('m_multisig_keys', bdxrpc.BlobFieldWrapper(x.ContainerType), type.SecretKey), obj=obj)
        return obj
