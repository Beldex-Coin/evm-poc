...
def bdx_type_to_type(elem_type):
    if isinstance(elem_type, int):
        return elem_type
    elif elem_type == object or elem_type == type:
        raise ValueError('Canot convert bdx type: %s' % elem_type)
    elif elem_type in bdxTypeMap:
        return bdxTypeMap[elem_type]
    elif isinstance(elem_type, x.bdxType) and elem_type.__class__ in bdxTypeMap:
        return bdxTypeMap[elem_type.__class__]
    elif len(elem_type.__bases__) > 0:
        return bdx_type_to_type(elem_type.__bases__[0])
    else:
        raise ValueError('Cannot convert bdx type %s' % elem_type)
...
class Blobber(object):
    ...
    async def container_dump(self, container, container_type, params=None):
        ...
        Blob array writer as in bdxRPC is serialized without size serialization
...
class Modeler(object):
    ...
    async def container_dump(self, container, container_type, params=None, obj=None):
        ...
        if container is None:  # todo: reconsider
            return NoSetSentinel()  # if not self.modelize else ArrayModel(obj, bdx_type_to_type(elem_type))
        ...
        return obj if not self.modelize else ArrayModel(obj, bdx_type_to_type(elem_type))
