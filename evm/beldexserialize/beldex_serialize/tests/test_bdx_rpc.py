...
class bdxRpcTest(aiounittest.AsyncTestCase):
    """Simple tests"""

    def __init__(self, *args, **kwargs):
        super(bdxRpcTest, self).__init__(*args, **kwargs)
    ...
    async def test_simple_msg(self):
        ...
        ar = bdxrpc.Archive(reader, False, modeled=True)
        ...
        arw = bdxrpc.Archive(writer, True)
        await arw.root()
        await arw.section(section)

        reader2 = x.MemoryReaderWriter(bytearray(data_bin))
        ar2 = bdxrpc.Archive(reader2, False, modeled=False)
