...
class bdxBoostTest(aiounittest.AsyncTestCase):
    """Simple tests"""

    def __init__(self, *args, **kwargs):
        super(bdxBoostTest, self).__init__(*args, **kwargs)
    ...
    async def test_destination_entries(self):
        ...
        data_bin = binascii.unhexlify(data_hex)
        reader = x.MemoryReaderWriter(bytearray(data_bin))
        ar = bdxb.Archive(reader, False, bdx.hf_versions(9))
        ...
        writer = x.MemoryReaderWriter()
        ar2 = bdxb.Archive(writer, True, bdx.hf_versions(9))
        await ar2.root()
        await ar2.message(msg)
    ...
    async def test_versions(self):
        ...
        writer = x.MemoryReaderWriter()
        ar2 = bdxb.Archive(writer, True, bdx.hf_versions(10))
        await ar2.root()
        await ar2.message(dest)

        reader = x.MemoryReaderWriter(writer.get_buffer())
        ar = bdxb.Archive(reader, False, bdx.hf_versions(10))
        msg = bdx.TxDestinationEntry()
        await ar.root()
        await ar.message(msg)
        ...
        ar2 = bdxb.Archive(writer2, True, bdx.hf_versions(9))
