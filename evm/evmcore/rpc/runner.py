...
class ContractHandler(object):
    ...
    async def handle_method_call(self,contract_address, contract_interaction, interaction, contract_type):
        ...
        funct(contract_interaction.args)
            ...
            try:
                if len(self.requests) == 0:
                    ...
                    for contract_interaction, contract_address in self.requests:
                        logging.info("Received Contract {} type : {}".format(contract_address, ContractType(contract_interaction.contract_type)))
                        if contract_interaction.contract_type == ContractType.Create:
                            ...
                            self.methodcalls.append(asyncio.ensure_future(self.handle_method_call(contract_address, contract_interaction, ContractType.Terminate)))
            ...
            try:
                ...
                if len(msg) == 3 and msg[0] == b'REPLY' and msg[1] in (b'_blocksub', b'_txallsub', b'_txflashesub') and msg[2] in (b'OK', b'ALREADY'):
                    ...
                    if len(msg)>2:
                        ...
                        print(otx)
                        if otx.txtype==5: #txtype_contract
                            contract = bdxdecoder.decodeContractRunner(otx.extra)
                            self.contracthandler.add_request(contract.contract, contract.contract_address.contract)
                else:
...
class EVMRunner(object):
    def __init__(self, connectionstring):
        ...
        if __debug__:
            ...
            globalcontract = Contract(None, dummy=True)
            ...
            globalcontract.bdx_address = b"ctr1Ncmfi8q8bzvdNvtUbnRUDPqHEyUM83rXwFbpqHVA9vcckHJ2fkM4U4wADYbCi7hh4ej1kjB8UVSKNVwMXDov3C4nqyeKdB"
            globalcontract.m_spend_secret_key = "7e57ad6e5aa9cb2e5e67e2ef7b33cf31031949ebdfb869296ce7cb66ee8d4f01"
            globalcontract.m_view_secret_key = "59acc8c77f695aa0ca191a4a82af0d3a37e94590b95d23a0062fc033bdab7707"
            globalcontract.amount = 1337
            globalcontract.state = 1
            globalcontract.contract_name = "Test Contract"
            self.evmserver.contracts[globalcontract.bdx_address] = globalcontract
